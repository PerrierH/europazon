...
class Data extends AbstractData
{
    ...
    public function encodeFriendlyUrl($originUrl)
    {
        ...
        if ($this->_getRequest()->getFullActionName()=='catalogsearch_result_index' || !$this->isEnabled() || strpos($originUrl, '?') === false) {
            return $originUrl;
        }
        ...
        [$url, $params] = explode('?', $correctUrl);
        $params = explode('&', $params);

        foreach ($params as $key => $param) {
            [$attKey, $attValues] = explode('=', $param);
