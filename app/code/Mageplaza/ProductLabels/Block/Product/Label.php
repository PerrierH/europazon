...
class Label extends AbstractLabel
{
     ...
     * @return string
     */
    public function getProductTooltip($productTooltip)
    {
        $productTooltip     = HelperData::jsonDecode($productTooltip);
        $storeCode          = $this->_helperData->getStore()->getCode();
        $productTooltipText = $this->escapeHtml(isset($productTooltip[$storeCode])
        && $productTooltip[$storeCode] ?
            $productTooltip[$storeCode] : ($productTooltip['admin'] ?? ''));

        return $productTooltipText;
    }
}
